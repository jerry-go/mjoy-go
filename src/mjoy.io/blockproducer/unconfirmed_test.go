...
func (r *noopHeaderRetriever) GetHeaderByNumber(number uint64) *block.Header {
	return nil
}
...
func TestUnconfirmedInsertBounds(t *testing.T) {
	...
	for depth := uint64(0); depth < 2*uint64(limit); depth++ {
		...
		for i := 0; i < int(depth); i++ {
			pool.Insert(depth, types.Hash([32]byte{byte(depth), byte(i)}))
		}
...
func TestUnconfirmedShifts(t *testing.T) {
	...
	for depth := start; depth < start+uint64(limit); depth++ {
		pool.Insert(depth, types.Hash([32]byte{byte(depth)}))
	}
