	...
	for zb0001 > 0 {
		...
		case "Priority":
			if dc.IsNil() {
				...
				if z.Priority == nil {
					z.Priority = new(types.BigInt)
				}
				err = z.Priority.DecodeMsg(dc)
				if err != nil {
					return
				}
			}
	...
	for zb0001 > 0 {
		...
		case "Priority":
			if msgp.IsNil(bts) {
				...
				bts, err = z.Priority.UnmarshalMsg(bts)
				if err != nil {
					return
				}
			}
