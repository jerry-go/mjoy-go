...
type Processor interface {
	Process(block *block.Block, statedb *state.StateDB, stateRootHash types.Hash) (*stateprocessor.DbCache, transaction.Receipts, []*transaction.Log, error)
}
...
// available in the database. It initialises the default mjoy Validator and
// Processor.
func NewBlockChain(chainDb database.IDatabase, config *params.ChainConfig, engine consensus.Engine) (*BlockChain, error) {
	...
	bc.SetProcessor(stateprocessor.NewStateProcessor(config,bc, engine))
...
func SetReceiptsData(config *params.ChainConfig, block *block.Block, receipts transaction.Receipts) {
	...
	for j := 0; j < len(receipts); j++ {
		...
		if len(transactions[j].Data.Actions)==2 && transactions[j].Data.Actions[1].Address == nil {
...
func (bc *BlockChain) insertChain(chain block.Blocks) (int, []interface{}, []*transaction.Log, error) {
	...
	for i, blk := range chain {
		...
		dbCache, receipts, logs, err := bc.processor.Process(blk, state, parent.Root())
		if err != nil {
		...
		case CanonStatTy:
			...
			logger.Debugf("\033[31m Inserted new block number:%d \033[0m  hash:%s  len(txs):%d elapsed:%d" , blk.Number().Int64() , blk.Hash().String() , len(blk.Transactions()) , common.PrettyDuration(time.Since(bstart)))
