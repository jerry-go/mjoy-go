...
type MjoylogError struct {
	error string
}

func (e MjoylogError) Error() string {
	return e.error
}

// error
var (
	ErrNewBackend        = MjoylogError{error: "New Backend failed"}
	ErrNoInstance        = MjoylogError{error: "Not create instance"}
	ErrLogRotatorFile    = MjoylogError{error: "Failed to create file rotator"}
	ErrLogRotatorCreated = MjoylogError{error: "Log has been Created"}
	ErrLogRotatorDir     = MjoylogError{error: "Failed to create log directory"}
)
