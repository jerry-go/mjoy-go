...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout mjoy_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for MjoySubscribe requests
}
...
// MjoySubscribe registers a subscripion under the "mjoy" namespace.
func (c *Client) MjoySubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "mjoy", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. MjoySubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through MjoySubscribe.
