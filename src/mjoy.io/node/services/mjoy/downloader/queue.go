...
type fetchRequest struct {
	...
	From    uint64              // [mjoy/62] Requested chain element index (used for skeleton fills only)
	Hashes  map[types.Hash]int //  [mjoy/61] Requested hashes with their insertion index (priority)
	Headers []*block.Header     // [mjoy/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead types.Hash // [mjoy/62] Hash of the last queued header to verify order

	// Headers are "special", they download in batches, supported by a skeleton chain
	headerTaskPool  map[uint64]*block.Header       // [mjoy/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [mjoy/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [mjoy/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [mjoy/62] Currently pending header retrieval operations
	headerResults   []*block.Header                // [mjoy/62] Result cache accumulating the completed headers
	headerProced    int                            // [mjoy/62] Number of headers already processed from the results
	headerOffset    uint64                         // [mjoy/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [mjoy/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[types.Hash]*block.Header // [mjoy/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [mjoy/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [mjoy/62] Currently pending block (body) retrieval operations
	blockDonePool  map[types.Hash]struct{}      // [mjoy/62] Set of the completed block (body) fetches

	receiptTaskPool  map[types.Hash]*block.Header // [mjoy/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [mjoy/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [mjoy/63] Currently pending receipt retrieval operations
	receiptDonePool  map[types.Hash]struct{}      // [mjoy/63] Set of the completed receipt fetches
