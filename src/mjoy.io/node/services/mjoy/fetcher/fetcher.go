...
type Fetcher struct {
	...
	fetchingHook       func([]types.Hash)     // Method to call upon starting a block (mjoy/61) or header (mjoy/62) fetch
	completingHook     func([]types.Hash)     // Method to call upon starting a block body fetch (mojy/62)
	importedHook       func(*block.Block)      // Method to call upon successful block import (both mjoy/61 and mjoy/62)
}
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.TxHash == block.DeriveSha(transaction.Transactions{}) {
