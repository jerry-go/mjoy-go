...
// PublicMjoyAPI provides an API to access Mjoy full node-related
// information.
type PublicMjoyAPI struct {
	e *Mjoy
}

// NewPublicMjoyAPI creates a new Mjoy protocol API for full nodes.
func NewPublicMjoyAPI(e *Mjoy) *PublicMjoyAPI {
	return &PublicMjoyAPI{e}
}


// Coinbase is the address that producing rewards will be send to (alias for Coinbase)
func (api *PublicMjoyAPI) Coinbase() (types.Address, error) {
	return api.Coinbase()
}

// Hashrate returns the POW hashrate
func (api *PublicMjoyAPI) Hashrate() hex.Uint64 {
	return hex.Uint64(api.e.Blockproducer().HashRate())
}
...
func NewPrivateBlockproducerAPI(e *Mjoy) *PrivateBlockproducerAPI {
...
// PrivateAdminAPI is the collection of Mjoy full node-related APIs
...
// admin methods of the Mjoy service.
func NewPrivateAdminAPI(mjoy *Mjoy) *PrivateAdminAPI {
	return &PrivateAdminAPI{mjoy: mjoy}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.mjoy.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.mjoy.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.mjoy.BlockChain().InsertChain(blocks); err != nil {
